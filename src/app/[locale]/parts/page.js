...
async function getPageData(locale) {
    return apiFetch(getLocalizedEndpoint("parts", locale), { cache: CACHE.NO_STORE });
}
                ...
                <MaintenanceSection
                    data={{
                        ...
                        dailyChecks: {
                            ...
                            items: extractListItems(whyChooseRaw.dailychecks?.[0]?.description),
                        },
                        monthlyChecks: {
                            title: whyChooseRaw.monthlychecks?.[0]?.title,
                            items: extractListItems(whyChooseRaw.monthlychecks?.[0]?.description),
                        },
                    }}
                />
            )}
