...
export default function ContactSection({ data }) {
  ...
  const buttonLink = data?.buttonLink?.url ?? data?.button?.link ?? "/";
        ...
        <div className="w-full bg-[#00095B] rounded-[8px] xl:rounded-[10px] 2xl:rounded-[12px] 3xl:rounded-[13px] p-[25px_20px] md:p-[25px] lg:p-[35px] xl:p-[45px] 2xl:p-[55px] 3xl:p-[70px] overflow-hidden ">
          <div className="w-full flex flex-wrap items-center justify-between gap-[10px]">
            <div className="max-w-full sm:max-w-[430px] xl:max-w-[480px] 2xl:max-w-[590px] 3xl:max-w-[760px] w-full">
              <Heading size="heading1" as="h2" className="text-white mb-[10px]">
                {data?.title}{" "}
              </Heading>
              <Text size="text1" as="p" className="text-white">
                {data?.description}{" "}
              </Text>
            </div>
            <Link
              ...
              className="3xl:text-[18px] 2xl:text-[14px] xl:text-[12px] text-[10px] font-bold text-white min-w-[130px] 3xl:min-w-[175px] h-[35px] 2xl:h-[50px] 3xl:h-[53px] flex items-center justify-center bg-[#1A73E8] px-6 rounded-full hover:bg-white hover:text-black transition cursor-pointer"
            >
              {buttonText}
            </Link>
          </div>
