...
//   const normalizeCar = (car, index) => {
//     const key = car?.slug || car?.modelName || `car-${index}`;
//     const name = car?.modelName || car?.slug || `Model ${index + 1}`;
...
//     const logoImage =
//       car?.modelBrand?.[0]?.featured_image?.url ||
...
//     const models = car?.modelBrand
//       ?.map((brand) => brand?.name)
//       .filter(Boolean) || [car?.modelName || "Standard"];

//     const colors =
//       car?.detail_page?.color_options?.[0]?.variations?.map((variation) => ({
//         code: variation?.color || "#cccccc",
...
//     const brochure =
//       car?.detail_page?.brochure?.url ||
//       car?.detail_page?.Banner?.[0]?.button_text ||
//       null;
...
//     () => carsList.map((car, index) => normalizeCar(car, index)),
//     [carsList]
//   );

//   const categories = useMemo(() => {
//     const allCategories = carsList.flatMap((car) =>
//       Array.isArray(car.modelCategory) ? car.modelCategory : []
//     );
...
//     if (!selectedCategory) return normalizedCars;
...
//     () => normalizedCars.find((car) => car.key === activeTab) || null,
//     [normalizedCars, activeTab]
//   );
...
//       setActiveTab(firstCar.key);
//       setSelectedModel(firstCar.models[0] || "");
//       setSelectedColorImage(firstCar.colors[0]?.image || firstCar.image || "");
//     } else {
//       setActiveTab("");
//       setSelectedModel("");
//       setSelectedColorImage("");
//     }
//     setIsVisible(true);
//   }, [selectedCategory, filteredCars]);

//   useEffect(() => {
//     if (currentCar) {
//       setSelectedModel(currentCar.models[0] || "");
//       setSelectedColorImage(
//         currentCar.colors[0]?.image || currentCar.image || ""
//       );
//     }
//   }, [activeTab, currentCar]);
...
//         <div className="flex flex-col lg:flex-row gap-10">
//           {/* Sidebar Categories - Only from API */}
//           <div className="w-full 2xl:w-[200px] lg:w-[160px] flex flex-wrap lg:flex-col m-[-3px] lg:my-[-7px]">
...
//                     className={`group 2xl:text-[14px] xl:text-[12px] 3xs:text-[11px] text-[10px] text-black h-[40px] w-full flex items-center gap-2 px-4 py-2 rounded-full border border-[#E8E8E8] transition-all cursor-pointer ${
...
//                 className="w-full"
//               >
//                 <TabsList className="bg-transparent gap-6 2xl:mb-[45px] sm:mb-[30px] mb-[15px] h-auto p-0 rtl:ml-auto flex-wrap">
//                   {filteredCars.map((car) => (
//                     <TabsTrigger
//                       key={car.key}
//                       value={car.key}
...
//                         {/* Mini image/video preview */}
//                         {isVisible &&
//                           currentCar &&
//                           currentCar.key === car.key && (
//                             <div
//                               className="absolute 2xl:top-[130px] top-[40%] sm:top-[100px] ltr:right-0 rtl:left-0 ltr:sm:right-[100px] rtl:xs:left-[50px] rtl:sm:left-[100px] w-full 2xl:max-w-[230px]
//                               lg:max-w-[200px] max-w-[150px] shadow-md"
//                             >
//                               <div className="relative">
...
//                                     <Image
//                                       src={
//                                         car.image || "/images/placeholder.png"
//                                       }
//                                       alt={car.name}
//                                       fill
//                                       style={{ objectFit: "contain" }}
...
//                         <div className="absolute sm:top-[-30%] top-[-25%] left-0 right-0 opacity-[0.1] m-auto w-full lg:max-w-[350px] max-w-[250px] z-[-1]">
//                           <Image
//                             src={car.logoImage || "/images/placeholder.png"}
//                             alt={car.name}
//                             width={400}
//                             height={500}
//                             className="w-full mx-auto object-contain"
                      ...
                      <div className="flex flex-col ltr:sm:flex-row rtl:sm:flex-row-reverse justify-between gap-6 sm:absolute top-0 left-0 rtl:text-right w-full">
                        <div className="sm:w-1/3">
                          <Heading
                            ...
                            className="text-black font-medium lg:mb-[35px] sm:mb-[25px] mb-[15px] max-w-[360px] rtl:ml-auto"
                          >
                            {currentCar.tagline || currentCar.name}
                          </Heading>
                          {currentCar.brochure && (
                            <Link
