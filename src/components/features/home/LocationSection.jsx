...
function LocationPin({ loading }) {
  return (
    <div className="w-[17px] h-[17px] transition-all group-hover:scale-[1.3] shrink-0">
      {loading ? (
        <div className="w-[17px] h-[17px] border-2 border-gray-400 border-t-[#1A73E8] rounded-full animate-spin" />
      ) : (
        <svg viewBox="0 0 17 17" className="group-hover:fill-[#1A73E8] fill-current">
...
function LocateBx({ title, description, tabIndex, locale }) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          // Use city/town/suburb — whatever is available
          const area =
            geo?.address?.city      ||
            geo?.address?.town      ||
            geo?.address?.suburb    ||
            ...
            "";
          if (area) {
            router.push(`${target}&search=${encodeURIComponent(area)}`);
          } else {
            // Fallback: pass raw coords, showroom page can handle
            router.push(`${target}&lat=${latitude}&lng=${longitude}`);
          }
        } catch {
          // Nominatim failed, still navigate with coords
          router.push(`${target}&lat=${latitude}&lng=${longitude}`);
        } finally {
          setLocLoading(false);
        }
  ...
  return (
    <div className="w-full max-w-[268px] sm:max-w-[320px] xl:max-w-[348px] 2xl:max-w-[418px] 3xl:max-w-[522px]">
      ...
      <Button
        ...
        disabled={locLoading}
        className="md:text-[14px] sm:text-[12px] text-[10px] font-medium w-full h-[40px] mb-[15px] relative flex items-center justify-start gap-2 transition-all shadow-0 rounded-[5px] px-[15px] bg-white text-gray-800 hover:bg-gray-100 cursor-pointer group hover:text-[#1A73E8] disabled:opacity-70"
      >
        <LocationPin loading={locLoading} />
        <span>{locLoading ? "Detecting location..." : "Use my current location"}</span>
      </Button>

      {error && (
        <p className="text-red-300 text-[10px] sm:text-[11px] mb-[10px] max-md:text-center">{error}</p>
      )}
      ...
      <form onSubmit={handleSearch} className="relative w-full">
        ...
        <Button
          type="submit"
          className="absolute top-0 ltr:right-[15px] rtl:left-[15px] bottom-0 my-auto w-[17px] h-[17px] cursor-pointer group"
        >
          <SearchIcon />
        </Button>
      </form>
...
export default function LocationSection({ data, locale = "en" }) {
  return (
    <section className="w-full bg-white 2xl:py-[75px_20px] xl:py-[60px_10px] py-[40px_10px] overflow-hidden">
      <div className="container">
        <div className="w-full p-[20px_15px] sm:p-[40px_30px] lg:p-[60px_40px] xl:p-[85px_96px] 2xl:p-[102px_115px] 3xl:p-[124px_144px] rounded-[8.8px] 2xl:rounded-[10.6px] 3xl:rounded-[13.3px] overflow-hidden flex flex-wrap justify-center md:justify-between gap-8 md:gap-5 relative z-0">
          ...
          <Image
            src={data?.background_image?.url || "/images/placeholder.png"}
            alt={data?.background_image?.alt || "locationImg"}
            width={965}
            height={632}
            className="w-full h-full object-cover duration-300 absolute -z-2 inset-0 pointer-events-none"
          />
          <LocateBx
            title={parse(data?.locate_showroom?.title || "")}
            description={parse(data?.locate_showroom?.short_description || "")}
            tabIndex={0}
            locale={locale}
          />
          <LocateBx
            title={parse(data?.locate_services_center?.title || "")}
            description={parse(data?.locate_services_center?.short_description || "")}
            tabIndex={1}
            locale={locale}
          />
        </div>
